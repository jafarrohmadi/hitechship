...
class UsersController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $query = User::with(['roles', 'email', 'terminals', 'ships'])->join('role_user', 'users.id', '=', 'role_user.user_id')->where('role_user.role_id', 3)->select(sprintf('%s.*', (new User)->table));
