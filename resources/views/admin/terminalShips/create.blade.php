...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.terminalShip.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.terminal-ships.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('terminals') ? 'is-invalid' : '' }}" name="terminals[]" id="terminals" multiple>
                    @foreach($terminals as $id => $terminal)
                        <option value="{{ $id }}" {{ in_array($id, old('terminals', [])) ? 'selected' : '' }}>{{ $terminal }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('arrive_time') ? 'is-invalid' : '' }}" type="text" name="arrive_time" id="arrive_time" value="{{ old('arrive_time') }}">
