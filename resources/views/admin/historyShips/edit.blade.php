    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.history-ships.update", [$historyShip->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('latitute') ? 'is-invalid' : '' }}" type="text" name="latitute" id="latitute" value="{{ old('latitute', $historyShip->latitute) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('logitude') ? 'is-invalid' : '' }}" type="text" name="logitude" id="logitude" value="{{ old('logitude', $historyShip->logitude) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('time_ship') ? 'is-invalid' : '' }}" type="text" name="time_ship" id="time_ship" value="{{ old('time_ship', $historyShip->time_ship) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('ships') ? 'is-invalid' : '' }}" name="ships[]" id="ships" multiple>
                    @foreach($ships as $id => $ship)
                        <option value="{{ $id }}" {{ (in_array($id, old('ships', [])) || $historyShip->ships->contains($id)) ? 'selected' : '' }}>{{ $ship }}</option>
                    @endforeach
                </select>
                @if($errors->has('ships'))
                    <div class="invalid-feedback">
